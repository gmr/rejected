...
class Client:
    ...
    def __init__(
        ...
        self._prefix: str = str(
            self._setting('prefix', self.DEFAULT_PREFIX)
        )
        ...
        try:
            if self._tcp_writer:
                ...
                self._udp_sock.sendto(
                    payload.encode('utf-8'), self._address
                )
        except OSError as error:  # pragma: nocover
            if self._connected:
                LOGGER.exception(
                    'Error sending statsd metric: %s', error
                )
    ...
    def _setting(
        ...
        if key not in self._settings:
            value = self._settings_in.get(
                key,
                os.environ.get(f'STATSD_{key}'.upper(), default),
            )
    ...
    def _tcp_socket(self) -> socket.socket | None:
        ...
        try:
            ...
            LOGGER.error(
                'Failed to connect via TCP, triggering shutdown: %s',
                error,
            )
            self._failure_callback()
            return None
        ...
        LOGGER.debug(
            'Connected to statsd at %s via TCP', self._address
        )

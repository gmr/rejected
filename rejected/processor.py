"""Base rejected Processor class that aims at simplifying the process of writing
processors that enforce good behaviors in dealing with messages.
...
try:
    ...
    logger.warning('pyyaml not found, disabling YAML support')
    yaml = None


class Processor(object):
    """Base Processor class to ease the implementation of strongly typed message
    processors that validate and automatically decode and deserialize the
    ...
    the processor will not process the message and will drop the message,
    returning True if Child._DROP_INVALID_MESSAGES is True. If it is False,
    the message will cause the processor to return False and return the message
    to the broker.
    ...
    def __init__(self, configuration):
        """Creates a new instance of a processor class. To perform
        initialization tasks, extend Processor._initialize
    ...
    def _initialize(self):
        """Extend this method for actions to take in initializing the Processor
    ...
    def _process(self):
        """Extend this method for implementing your processor logic.

        If the message can not be processed and the processor should stop after
        n failures to process messages, raise the ProcessingException.
    ...
    def _get_configuration_obj(self, config):
        ...
        defined in normal, forward DNS notation (messaging.scs.meetme.com).
    ...
    def _get_service(self, fqdn):
        """Return the service notation for the fqdn value.

        :param str fqdn: The fqdn value (messaging.scs.myyearbook.com, etc)
        :rtype: str
    ...
    def _load_html_value(self, value):
        ...
        """
        if not bs4:
            raise ProcessingException('BeautifulSoup is not enabled')
    ...
    def _load_yaml_value(self, value):
        ...
        """
        if not bs4:
            raise ProcessingException('YAML is not enabled')
    ...
    def process(self, message_in):
        ...
        a message, extend Processor._process, not this method.

        :param rejected.Processor.Message message_in: The message to process
        :rtype: bool
        ...
        if self._MESSAGE_TYPE and self._MESSAGE_TYPE != self.message_type:
            ...
            logger.warning('Returning invalid message and indicating an error')
            return False

        # Will only return False if a ProcessingException is thrown
        try:
            self._process()
            return True
        except ProcessingException as error:
            logger.warning('Received processing exception: %s', error)
            return False


class ProcessingException(Exception):
    """May be called when processing a message to indicate a problem that the
    processor may be experiencing that should cause it to stop.
